...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<any> {
    const dataSub = new Subject();
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=472527d75985263d65447cf8f08e1e29`)
      .subscribe((res) => {
        //let data = res as WeatherData
        dataSub.next(res);
      }, (err) => {
        console.log(err);
      });
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=472527d75985263d65447cf8f08e1e29`)
      .subscribe((data) => {
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number>  {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=472527d75985263d65447cf8f08e1e29`)
      .subscribe((weather: any) => {
